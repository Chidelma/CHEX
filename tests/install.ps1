...
#
# The mirror of tests/install.sh. Invoke-WebRequest has no file:// support, so
# the harness shadows it with a function that serves a fake release from a temp
# directory -- no network, no listener. Only $releases, $dest, and the PATH
# write are rewritten, so the verification block under test is the real one.
# Every failure case here installed an unverified binary before the checks were
# made to fail closed.
...
$work = Join-Path ([System.IO.Path]::GetTempPath()) ("chex-install-" + [guid]::NewGuid())
...
# Rewrite the three environment-specific lines, leaving the logic untouched.
$script = Get-Content -Raw $src
$script = $script -replace '(?m)^\$releases\s*=.*$', ('$releases = ' + "'$releases'")
...
Rename-Item (Join-Path $latest 'SHA256SUMS') 'SHA256SUMS.hidden'
Check 'CHEX_SKIP_CHECKSUM=1 installs' 0 (Invoke-Installer @{ CHEX_SKIP_CHECKSUM = '1' })
...
Check 'CHEX_VERSION installs the pinned release' 0 (Invoke-Installer @{ CHEX_VERSION = $pinnedTag })
...
Check 'CHEX_VERSION without a leading v works' 0 (Invoke-Installer @{ CHEX_VERSION = $pinnedTag.TrimStart('v') })
Assert '  the pinned binary is the one installed' ((Payload) -eq 'pinned binary')
Remove-Item $exe -Force -ErrorAction SilentlyContinue

Check 'unknown CHEX_VERSION aborts' 1 (Invoke-Installer @{ CHEX_VERSION = 'v0.0.00' })
Assert '  nothing is installed' (-not (Installed))
