#!/bin/sh
# CHEX installer for macOS and Linux.
...
# Defaults to the latest release. Set CHEX_VERSION to install a specific one --
# this is the rollback path when a release turns out bad:
#   CHEX_VERSION=26.28.02 curl -fsSL .../install.sh | sh
# Tags carry a leading "v", which is added here when it is left off.
if [ -n "${CHEX_VERSION:-}" ]; then
    tag="$CHEX_VERSION"
...
if ! curl -fSL "$url" -o "$tmp/chex"; then
    echo "Failed to download ${asset} from ${url}" >&2
    if [ -n "${CHEX_VERSION:-}" ]; then
        echo "Is CHEX_VERSION=${CHEX_VERSION} a released version? See ${RELEASES}" >&2
    fi
    exit 1
fi
...
# Set CHEX_SKIP_CHECKSUM=1 to install without verification.
if [ "${CHEX_SKIP_CHECKSUM:-}" = "1" ]; then
    echo "Warning: skipping checksum verification (CHEX_SKIP_CHECKSUM=1)." >&2
else
    if command -v sha256sum >/dev/null 2>&1; then
        ...
        echo "Install one, or re-run with CHEX_SKIP_CHECKSUM=1 to bypass." >&2
        exit 1
    fi
    ...
    actual=$($hash_cmd "$tmp/chex" | awk '{print $1}')
...
chmod +x "$tmp/chex"
mv "$tmp/chex" "$dest/chex"

echo "Installed chex to ${dest}/chex"
case ":$PATH:" in
    *":$dest:"*) : ;;
    *) echo "Note: ${dest} is not on your PATH. Add it, e.g.:"; echo "  export PATH=\"${dest}:\$PATH\"" ;;
esac
echo "Run 'chex --help' to get started."
